...
4. Set this up as npm package so that you can type `jolt run dev` or something, and the function's server will spin up in the background while the react ap is running in your browser
  - Add `proxy PORT_NUMBER` to React package.json
  - Configure the JOLT config file to support a script that starts the app using the specified dev command, starts the function server and proxies requests to functions appropriately
